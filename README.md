...
| `layerOp`     | float array (length =`layers`) | Opacity multiplier per depth plane (near/mid/far).                 |
...
- **3 depth planes** — Near (full size, bright), mid (85% size, dimmer), far (70% size, faintest). Rendered back-to-front.
- **Deletion streams** — ~15% of streams erase characters as they pass, creating organic density cycles.
