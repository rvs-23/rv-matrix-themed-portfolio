 ...
 * (e.g., appendToTerminal, userDetails, theme functions, loaded JSON data).
...
function getTerminalCommands(context) {
    const {
        ...
        // --- NEW: Destructure loaded data from context ---
        skillsData,  // This will now come from skills.json via context
        hobbiesData, // This will come from hobbies.json via context
        manPages     // This will come from manPages.json via context (for future 'man' command)
    } = context;

    // --- OLD HARDCODED skillsData IS REMOVED ---
    // It's now passed via `skillsData` in the context from matrix-rain.js

    /**
     * @function renderSkillTree
     * @description Renders a hierarchical tree structure for skills or hobbies.
     * @param {object} node - The current node in the data structure to render.
     * @param {string} [indent=''] - Current indentation string.
     * @param {boolean} [isLast=true] - Whether this node is the last child of its parent.
     * @param {string[]} [outputArray=[]] - Array to accumulate HTML output lines.
     * @param {boolean} [isRootCall=true] - Whether this is the initial call for a root node.
     * @returns {string[]} The array of HTML output lines.
     */
    function renderSkillTree(node, indent = '', isLast = true, outputArray = [], isRootCall = true) {
        if (!node || typeof node.name === 'undefined') {
            const errorMsg = isRootCall && !node ? "Error: Data source is not loaded or empty." : "Error: Malformed data node.";
            console.error("Error in renderSkillTree:", errorMsg, "Node:", node);
            // Ensure outputArray is always initialized
        ...
        if (node.children && node.children.length > 0) {
            node.children.forEach((child, index) => {
                renderSkillTree(child, newIndent, index === node.children.length - 1, outputArray, false);
            });
        }
        return outputArray;
    }
    ...
    async function activateTerminalGlitchAndQuote() {
        ...
        if (!terminalOutput || !context.mainContentContainer) return; // context.mainContentContainer comes from the outer scope
        ...
        const currentRainCfg = context.getRainConfig ? context.getRainConfig() : {}; // context.getRainConfig from outer scope
        ...
        let glitchInterval = setInterval(() => {
            ...
            for (let i = 0; i < lines; i++) {
                for (let j = 0; j < charsPerLine; j++) {
                    // context.allMatrixChars from outer scope
        ...
        function finalizeEasterEgg() {
            ...
            // context.fullWelcomeMessageString & context.userDetails from outer scope
    ...
    const rainPresets = {
        'default': {
            ...
            action: () => {
                if (typeof resetRainConfig === 'function') { // resetRainConfig from outer context
                    resetRainConfig();
    ...
    const terminalCommands = {
        ...
        'about': () => {
            const parts = {};
            fullBioText.split('\n').forEach(line => { // fullBioText from outer context
            ...
            const name = parts.name || userDetails.userName; // userDetails from outer context
            ...
            appendToTerminal(htmlOutput, 'output-about-wrapper'); // appendToTerminal from outer context
        },
        ...
        'clear': () => {
            ...
            appendToTerminal(fullWelcomeMessageString.replace(/\n/g, '<br/>'), 'output-welcome-wrapper'); // fullWelcomeMessageString from outer context
        },
        ...
        /**
         * @command help
         * @description MODIFIED: Added 'hobbies' to the list.
         */
        'help': () => {
            ...
            let commandList = [
                ...
                // --- NEW: Added hobbies to help output ---
                { cmd: "hobbies", display: "hobbies", desc: "List my hobbies and interests." },
                // { cmd: "man <command>", display: "man <command>", desc: "Show detailed manual for a command." }, // Future
        ...
        /**
         * @command hobbies
         * @description Displays a summary of hobbies and interests from hobbies.json.
         */
        'hobbies': () => {
            ...
            // Use root name from JSON if available, otherwise default
            ...
            if (hobbiesData.children && hobbiesData.children.length > 0) {
                hobbiesData.children.forEach(category => {
                    htmlOutput += `<div class="output-skills-category">${category.name.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>`; // Re-use skills styling for consistency
                    ...
                    if (category.children && category.children.length > 0) {
                        ...
                        htmlOutput += `<li>Details for ${category.name.replace(/</g, "&lt;").replace(/>/g, "&gt;")} (use 'hobbytree ${category.aliases ? category.aliases[0] : category.name.toLowerCase().split(' ')[0]}' for more)</li>`;
                    }
                    htmlOutput += `</ul>`;
                });
            } else {
            ...
            appendToTerminal(htmlOutput, 'output-skills-wrapper'); // Can reuse 'output-skills-wrapper' or create a new one like 'output-hobbies-wrapper'
        },
        ...
        'rainpreset': (args) => {
            ...
            if (presetName === 'default' && typeof presetData.action === 'function') {
            ...
            } else if (presetData.config && typeof updateRainConfig === 'function') { // updateRainConfig from outer context
                ...
                for (const param of applyOrder) {
                    if (presetData.config.hasOwnProperty(param)) {
                        if (updateRainConfig(param, presetData.config[param], presetData.config)) successCount++; else errorCount++;
        ...
        'resize': (args) => {
            if (args[0] && args[0].toLowerCase() === 'term') {
                if (args.length === 2 && args[1].toLowerCase() === 'reset') {
                    if (typeof resizeTerminalElement === 'function' && defaultTerminalSize) { // resizeTerminalElement, defaultTerminalSize from outer context
        ...
        /**
         * @command skills
         * @description MODIFIED: Displays a summary of key skill areas from skills.json.
         */
        'skills': () => {
                ...
                skillsData.children.forEach(category => {
                    ...
                    if (category.children && category.children.length > 0) {
                        ...
                        if (category.children.length > 3) {
                            const aliasToSuggest = category.aliases && category.aliases.length > 0 ? category.aliases[0] : category.name.toLowerCase().split(' ')[0];
                            htmlOutput += `<li>... and more (use 'skilltree ${aliasToSuggest}')</li>`;
                        }
                    } else {
                        htmlOutput += `<li>(Details for ${category.name.replace(/</g, "&lt;").replace(/>/g, "&gt;")} via 'skilltree')</li>`;
                    }
                    htmlOutput += `</ul>`;
                });
            } else {
        ...
        /**
         * @command skilltree
         * @description MODIFIED: Explore skills from skills.json using paths and aliases.
         */
        'skilltree': (args) => {
            if (!skillsData) { // Check if skillsData (from JSON) is loaded
                appendToTerminal("<div class='output-error'>Skills data not loaded. Please check 'assets/skills.json'.</div>", 'output-error-wrapper');
                return;
            }
            ...
            if (pathArg) {
                const pathParts = pathArg.toLowerCase().split('>').map(p => p.trim()).filter(p => p);
                ...
                for (const part of pathParts) {
                    ...
                    if (currentNode.children) {
                        foundNodeInChildren = currentNode.children.find(child => {
                            const nameMatch = child.name.toLowerCase() === part;
                            // Ensure aliases exist and map them to lowercase for case-insensitive matching
                            const aliasMatch = child.aliases && Array.isArray(child.aliases) && child.aliases.map(a => String(a).toLowerCase()).includes(part);
                    ...
                    if (foundNodeInChildren) {
                        ...
                        appendToTerminal(`<div>Hint: Check available aliases or names. Ensure aliases are defined in 'assets/skills.json'. Current skill root: '${skillsData.name || "Skills"}'.</div>`, 'output-text-wrapper');
                        pathFound = false;
                        break;
                    }
                }
                if (pathFound) {
                    ...
                    displayPath = (skillsData.name || "Skills Root") + (currentPathForDisplayArray.length > 0 ? " > " + currentPathForDisplayArray.join(' > ') : "");
                } else {
                    return; // Exit if path not found
                }
            }

            // Initialize outputArray for renderSkillTree
            const outputArrayForTree = [];
            ...
            let treeHtmlLines = [];
            if (targetNode === skillsData) { // Root call or empty path argument
                if (targetNode.children && targetNode.children.length > 0) {
                    targetNode.children.forEach((child, index) => renderSkillTree(child, '  ', index === targetNode.children.length - 1, treeHtmlLines, false));
                } else {
                    treeHtmlLines.push("  (No skill categories defined under root)");
                }
            } else if (targetNode.children && targetNode.children.length > 0) { // Navigated to a category with children
                targetNode.children.forEach((child, index) => renderSkillTree(child, '  ', index === targetNode.children.length - 1, treeHtmlLines, false));
            } else { // Navigated to a leaf node or an empty category
                treeHtmlLines.push("    └── (End of this skill branch or no further sub-skills listed.)");
                // You could add a description field to leaf nodes in your JSON for more detail
                // if (targetNode.description) { treeHtmlLines.push(`        ${targetNode.description.replace(/</g, "&lt;").replace(/>/g, "&gt;")}`); }
            }

            const fullOutput = titleHtml + treeHtmlLines.join('\n').replace(/\n/g, '<br/>');
            appendToTerminal(fullOutput, 'output-skilltree-wrapper');

            if (!pathArg && skillsData.children && skillsData.children.length > 0) {
                appendToTerminal("<div>Hint: Navigate deeper using names or aliases (e.g., `skilltree se`, `skilltree ai > genai`). Check `assets/skills.json` for defined aliases.</div>", "output-text-wrapper");
            }
        },
        ...
        'termopacity': (args) => {
            ...
            let newOpacity;
            if (input === 'reset') {
                newOpacity = initialTerminalOpacity; // initialTerminalOpacity from outer context
        ...
        'theme': (args) => {
            ...
            if (allValidOptions.includes(themeNameInput)) {
                ...
                if (typeof restartMatrixRain === 'function') { // restartMatrixRain from outer context
                    restartMatrixRain();
                }
            } else {
        ...
        'toggleterm': () => {
            if (typeof toggleTerminal === 'function') { // toggleTerminal from outer context
                toggleTerminal();
            } else {
