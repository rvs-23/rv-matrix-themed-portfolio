...
let terminalOutputEl, commandInputEl, mainContentContainerEl, promptArrowEl;
let commandHistory = [];
let historyIndex = 0;
...
export function initializeTerminalController(config, commands, commandContextFunc) {
    ...
    promptArrowEl = document.querySelector('.prompt-arrow'); // Assuming there's one

    defaultTerminalSizeConfig = config.terminalConfig.defaultTerminalSize || defaultTerminalSizeConfig;
    initialTermOpacityConfig = config.terminalConfig.initialTerminalOpacity || initialTermOpacityConfig;
    ...
    document.documentElement.style.setProperty('--terminal-opacity', String(initialTermOpacityConfig));
...
function handleCommandInputKeydown(e) {
    ...
    if (e.key === 'Enter') {
        ...
        if (fullCommandText) {
            ...
            appendToTerminal(`<div><span class="prompt-arrow">&gt;</span> <span class="output-command">${sanitizedCommandDisplay}</span></div>`);
        ...
        if (commandHistory.length > 0) {
            ...
            setTimeout(() => commandInputEl.setSelectionRange(commandInputEl.value.length, commandInputEl.value.length), 0);
        }
...
async function processCommand(fullCommandText) {
    ...
    if (typeof commandFunc === 'function') {
        try {
            ...
            appendToTerminal(`<div class="output-error">Command Error: ${err.message || 'Unknown error'}</div>`);
        }
    } else if (commandName) {
        appendToTerminal(`<div class="output-error">Command not found: ${commandName.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>`);
        appendToTerminal(`<div>Type 'help' for a list of available commands.</div>`);
    }
}
...
export function displayInitialWelcomeMessage() {
    if (terminalOutputEl && fullWelcomeMsg) {
        appendToTerminal(fullWelcomeMsg.replace(/\n/g, '<br/>'), 'output-welcome-wrapper');
    }
}
...
export function resizeTerminalElement(width, height) {
    if (mainContentContainerEl) {
        ...
        appendToTerminal(`<div class='output-success'>Terminal resized to ${width} width, ${height} height.</div>`);
    } else {
        appendToTerminal("<div class='output-error'>Terminal container element not found for resize.</div>");
    }
}
...
export function setTerminalOpacity(opacityValue) {
    let newOpacity;
     if (opacityValue === 'reset') {
        ...
        if (isNaN(parsedInput)) {
            appendToTerminal("<div class='output-error'>Invalid opacity value. Must be a number (e.g., 75 or 0.75) or 'reset'.</div>");
            return;
        }
        if (parsedInput > 1 && parsedInput <= 100) {
            ...
            appendToTerminal("<div class='output-error'>Opacity value out of range. Must be 0-100 or 0.0-1.0.</div>");
            return;
        }
    }
    ...
    appendToTerminal(`<div class='output-success'>Terminal opacity set to ${(newOpacity * 100).toFixed(0)}%.</div>`);
}
...
export function setTerminalFontSize(sizeInput) {
    ...
    if (inputSize === 'small') newSize = '10.5px';
    else if (inputSize === 'default') newSize = '12.5px'; // Assuming a default from your CSS
        ...
        if (inputSize.endsWith('px') && (sizeValue < 7 || sizeValue > 28)) {
            appendToTerminal("<div class='output-error'>Pixel size out of reasonable range (7px-28px).</div>"); return;
        }
        newSize = inputSize;
    } else {
        appendToTerminal("<div class='output-error'>Invalid size. Use 'small', 'default', 'large', or a value like '10px', '1.2em'.</div>"); return;
    }
    document.documentElement.style.setProperty('--terminal-font-size', newSize);
    appendToTerminal(`<div class='output-success'>Terminal font size set to ${newSize}.</div>`);
}
...
export function getCurrentThemeColors() {
    ...
    return { primary: '#0F0', secondary: '#00FFFF', glow: '#9FFF9F', background: '#000' };
}
...
export function applyTheme(themeNameInput) {
    ...
    const validSpecificThemes = [
        ...
        'retroarcade', 'reloaded', 'synthwavegrid', 'voidblue'
    ];

    const showThemeUsage = () => {
        const currentThemeClass = Array.from(document.body.classList).find(cls => cls.startsWith('theme-')) || 'theme-green'; // Default to green
        appendToTerminal("<div class='output-error'>Usage: theme &lt;name&gt;</div>");
        appendToTerminal(`<div>Available themes: ${validSpecificThemes.sort().join(', ')}.</div>`); // Removed 'dark' from here
        appendToTerminal(`<div>Current theme: ${currentThemeClass.replace('theme-', '')}</div>`);
    };
    ...
    if (validSpecificThemes.includes(themeNameInput)) {
        ...
        appendToTerminal(`<div class='output-success'>Theme set to ${targetThemeClass.replace('theme-', '')}.</div>`);
        ...
        appendToTerminal(`<div class='output-error'>Error: Theme "${themeNameInput.replace(/</g, "&lt;").replace(/>/g, "&gt;")}" not found.</div>`);
        showThemeUsage();
