...
export default function downloadCvCommand(args, context) {
  ...
  if (args[0] && args[0].toLowerCase() === "cv") {
    if (
      !userConfig.cvLink ||
      userConfig.cvLink === "" ||
      userConfig.cvLink === "path/to/your/resume.pdf"
    ) {
      // More robust check
      return appendToTerminal(
        "<div class='output-error'>CV link not configured or is a placeholder.</div>",
        "output-error-wrapper",
      );
    }

    appendToTerminal(
      "<div>Attempting to prepare CV for viewing/download...</div>",
      "output-text-wrapper",
    );
    ...
    if (match && match[1]) {
      ...
      appendToTerminal(
        `<div>Using Google Drive direct download link format.</div>`,
        "output-text-wrapper",
      );
    } else {
      appendToTerminal(
        `<div>Using provided link directly. Behavior depends on link type.</div>`,
        "output-text-wrapper",
      );
    }
    ...
    const userNameForFile = (userConfig.userName || "User").replace(
      /\s+/g,
      "_",
    );
    ...
    appendToTerminal(
      `<div>If your browser blocked the pop-up or download didn't start, check your browser settings. You can also use the CV link in the navigation bar.</div>`,
      "output-text-wrapper",
    );
  } else {
    appendToTerminal(
      "<div class='output-error'>Usage: download cv</div>",
      "output-error-wrapper",
    );
  }
}
