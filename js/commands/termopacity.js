...
export default function termOpacityCommand(args, context) {
  ...
  if (!args || args.length === 0) {
    ...
    appendToTerminal(
      "<div class='output-error'>Usage: termopacity &lt;value&gt; (0-100 or 0.0-1.0) or 'termopacity reset'</div>",
      "output-error-wrapper",
    );
    appendToTerminal(
      `<div>Current terminal opacity: ${currentOpacityPercentage}</div>`,
      "output-text-wrapper",
    );
    return;
  }
  ...
  if (terminalController.setTerminalOpacity) {
    ...
    appendToTerminal(
      "<div class='output-error'>Terminal opacity control not available.</div>",
      "output-error-wrapper",
    );
  }
}
