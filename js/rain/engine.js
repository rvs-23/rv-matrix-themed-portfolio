...
import { getActiveRainConfig, getRainGlyphs, setActiveRainColors } from './config.js';
...
class Stream {
    ...
    reset() {
        ...
        this.opacity = CFG.layerOp && CFG.layerOp[this.layer] !== undefined ? CFG.layerOp[this.layer] : 1;
    ...
    step() {
        ...
        if (++this.tick >= activeCfg.trailMutate) {
            this.tick = 0;
            for (let r = 0; r < ROWS; r++) if (Math.random() < 0.15 && this.buf[r]) this.buf[r] = randChar();
        }
