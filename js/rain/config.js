...
export function initializeRainConfig(loadedRainConfig) {
    defaultRainConfig = loadedRainConfig.defaultConfig || {
        speed: 100, font: 15, baseCol: "#0F0", headCol: "#FFF", fontFamily: "monospace", layers: 1, layerOp: [1], density: 0.7, minTrail: 10, maxTrail: 30, headGlowMin: 1, headGlowMax: 5, blur: 0, trailMutate: 150, fade: 0.1, decayBase: 0.9, delChance: 0
    };
...
export function setActiveRainColors(baseColor, headColor) {
    ...
    console.log(`Rain colors updated in active config: base=${activeRainConfig.baseCol}, head=${activeRainConfig.headCol}`);
}


export function updateRainConfigParameter(param, value, applyingPresetContext = null) {
    const MAPPINGS = {
    ...
    if (!setting) {
        ...
        if (param === 'baseCol' || param === 'headCol') {
            if (typeof value === 'string' && (value.startsWith('#') || value.startsWith('rgb'))) { // Basic validation
    ...
    switch (setting.type) {
        ...
        case 'string_font':
            ...
            break;
        case 'array_float':
            parsedValue = Array.isArray(value) ? value.map(Number).filter(n => !isNaN(n)) : [];
            if (setting.key === 'layerOp' && parsedValue.length !== (applyingPresetContext?.layers ?? activeRainConfig.layers)) {
                console.warn(`RainConfig: LayerOp length mismatch. Expected ${applyingPresetContext?.layers ?? activeRainConfig.layers}, got ${parsedValue.length}`);
                isValid = false;
            } else {
    ...
    if (!isValid) {
        console.warn(`RainConfig: Invalid value for ${param}: '${value}'. Check type, range, or format.`);
        return false;
    }
    
    // Validation for min/max trail and headGlow
    if (setting.key === 'minTrail' && parsedValue > (applyingPresetContext?.maxTrail ?? activeRainConfig.maxTrail)) isValid = false;
    if (setting.key === 'maxTrail' && parsedValue < (applyingPresetContext?.minTrail ?? activeRainConfig.minTrail)) isValid = false;
    if (setting.key === 'headGlowMin' && parsedValue > (applyingPresetContext?.headGlowMax ?? activeRainConfig.headGlowMax)) isValid = false;
    if (setting.key === 'headGlowMax' && parsedValue < (applyingPresetContext?.headGlowMin ?? activeRainConfig.headGlowMin)) isValid = false;

    if (!isValid) {
        console.warn(`RainConfig: Validation failed for ${param} (e.g. minTrail > maxTrail). Value: '${value}'`);
        return false;
    }
...
export function resetRainConfigToActiveDefaults() {
    activeRainConfig = { ...defaultRainConfig };
    console.log("Rain configuration reset to loaded defaults (from config/rain.json).");
}